...
export function validateManifest(manifest: unknown): ManifestValidationError[] {
  ...
  if (!isNonEmptyString(m.schemaVersion) || !SEMVER.test(m.schemaVersion as string)) {
    ...
    if (major !== currentMajor) {
      errors.push({ field: 'schemaVersion', message: `Incompatible major version. Expected ${currentMajor}.x.x.` });
    }
  }
  ...
  if (!isObject(m.project)) {
    ...
    if (!isNonEmptyString(p.createdAt)) errors.push({ field: 'project.createdAt', message: 'Required.' });
    if (!Array.isArray(p.sessionIds)) errors.push({ field: 'project.sessionIds', message: 'Required. Must be an array.' });
    if (typeof p.sessionCount !== 'number') errors.push({ field: 'project.sessionCount', message: 'Required. Must be a number.' });
  }
  ...
  if (!isObject(m.ledger)) {
    ...
    if (typeof l.eventCount !== 'number') errors.push({ field: 'ledger.eventCount', message: 'Required. Must be a number.' });
    if (!Array.isArray(l.eventTypes)) errors.push({ field: 'ledger.eventTypes', message: 'Required. Must be an array.' });
    if (!isNonEmptyString(l.fileHash)) errors.push({ field: 'ledger.fileHash', message: 'Required. Must be a SHA-256 hex string.' });
    if (typeof l.fileSize !== 'number') errors.push({ field: 'ledger.fileSize', message: 'Required. Must be a number.' });
    if (!isNonEmptyString(l.eventSchemaVersion)) errors.push({ field: 'ledger.eventSchemaVersion', message: 'Required.' });
  }
  ...
  if (!isObject(m.integrity)) {
    ...
    if (i.algorithm !== 'sha256') errors.push({ field: 'integrity.algorithm', message: 'Must be "sha256".' });
    if (!Array.isArray(i.files)) errors.push({ field: 'integrity.files', message: 'Required. Must be an array.' });
    if (!isNonEmptyString(i.computedAt)) errors.push({ field: 'integrity.computedAt', message: 'Required.' });
    ...
    if (Array.isArray(i.files)) {
      for (let idx = 0; idx < (i.files as unknown[]).length; idx++) {
        ...
        if (!isNonEmptyString(file.path)) errors.push({ field: `integrity.files[${idx}].path`, message: 'Required.' });
        if (!isNonEmptyString(file.hash)) errors.push({ field: `integrity.files[${idx}].hash`, message: 'Required.' });
        if (typeof file.size !== 'number') errors.push({ field: `integrity.files[${idx}].size`, message: 'Required.' });
      }
    }
  }
  ...
  if (m.state !== null && m.state !== undefined) {
    if (!isObject(m.state)) {
      ...
      if (!isNonEmptyString(s.path)) errors.push({ field: 'state.path', message: 'Required when state is present.' });
      if (typeof s.stateVersion !== 'number') errors.push({ field: 'state.stateVersion', message: 'Required when state is present.' });
      if (typeof s.activeStatements !== 'number') errors.push({ field: 'state.activeStatements', message: 'Required when state is present.' });
      if (!isNonEmptyString(s.fileHash)) errors.push({ field: 'state.fileHash', message: 'Required when state is present.' });
    }
  }
  ...
  if (m.artifacts !== null && m.artifacts !== undefined) {
    if (!isObject(m.artifacts)) {
      ...
      if (!isNonEmptyString(a.registryPath)) errors.push({ field: 'artifacts.registryPath', message: 'Required when artifacts is present.' });
      if (typeof a.totalArtifacts !== 'number') errors.push({ field: 'artifacts.totalArtifacts', message: 'Required when artifacts is present.' });
      if (!isNonEmptyString(a.registryHash)) errors.push({ field: 'artifacts.registryHash', message: 'Required when artifacts is present.' });
    }
  }
