...
export function registerContextCommand(program: Command): void {
  ...
  ctx
    ...
    .option('-m, --model <preset>', 'Model preset for auto budget (e.g. claude-sonnet, gpt-4o, small)')
    ...
    .action((opts) => {
      ...
      const pkg = buildContextPackage({
        workspaceRoot: opts.root, projectId,
        tokenBudget: budget, layers,
        focusTopic: opts.focus,
        maxEvidenceEvents: opts.maxEvidence,
        maxArchiveEvents: opts.maxArchive,
      });

      if (opts.raw) { console.log(pkg.combined); return; }
  ...
  ctx
    ...
    .action((layerId: string, opts) => {
      ...
      if (opts.raw) { console.log(content.content); return; }
  ...
  ctx
    ...
    .action((opts) => {
      ...
      const pkg = buildContextPackage({
        workspaceRoot: opts.root, projectId, tokenBudget: budget,
  ...
  ctx
    ...
    .action(() => {
      console.log('\n  Model Presets:\n');
      console.log(`  ${'ID'.padEnd(18)} ${'Name'.padEnd(22)} ${'Window'.padEnd(10)} ${'Usable'.padEnd(10)}`);
      console.log(`  ${'─'.repeat(18)} ${'─'.repeat(22)} ${'─'.repeat(10)} ${'─'.repeat(10)}`);

      for (const p of MODEL_PRESETS) {
        console.log(`  ${p.id.padEnd(18)} ${p.name.padEnd(22)} ${String(p.contextWindow).padEnd(10)} ${String(p.usableBudget).padEnd(10)}`);
      }
